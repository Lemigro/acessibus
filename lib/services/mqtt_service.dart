...
class MqttService {
  ...
  static const String _topicoParadasSelecao = 'paradas/selecao'; // Seleção de botões
  static const String _topicoOnibusChegando = 'onibus/chegando'; // Notificação de chegada
  ...
  Function(String linha, String tipoDeficiencia, String paradaId)? _onSelecaoBotao;
  Function(String linha, String paradaId)? _onOnibusChegando; // Callback para aviso de chegada
  
  // Armazena seleções de botões por parada
  Map<String, Map<String, String>> _selecoesParadas = {}; // {paradaId: {linha, tipo}}
  ...
  void onAlertaBotao(Function(String linha, String tipoDeficiencia, String alerta) callback) {
  ...
  void onSelecaoBotao(Function(String linha, String tipoDeficiencia, String paradaId) callback) {
  ...
  Future<bool> publicarChegadaOnibus(String paradaId, String linha) async {
    ...
    try {
      ...
      print('MQTT: ✅ Publicado chegada do ônibus - Parada: $paradaId, Linha: $linha');
      print('MQTT: O dispositivo físico $paradaId deve receber o alerta agora!');
      return true;
    } catch (e, stackTrace) {
  ...
  Future<bool> conectar() async {
    ...
    try {
      ...
      if (_client!.connectionStatus!.state == MqttConnectionState.connected) {
        _conectado = true;
        print('MQTT: ✅ Conectado com sucesso ao broker $_brokerValue:$_portValue');
        return true;
      } else {
        print('MQTT: ❌ Falha ao conectar. Estado: ${_client!.connectionStatus?.state}');
  ...
  Future<bool> iniciarMonitoramento({
    ...
    try {
      ...
      print('MQTT: Subscrito ao tópico de localização (wildcard): $topicoLocalizacaoWildcard');
      ...
      if (linha != null && linha.isNotEmpty) {
        ...
        print('MQTT: Subscrito ao tópico de localização específico: $topicoLocalizacao');
      }
      ...
      _client!.updates!.listen(
        (List<MqttReceivedMessage<MqttMessage?>>? c) {
          print('MQTT: 🔔 Listener acionado! Lista recebida: ${c?.length ?? 0} mensagens');
          ...
          for (int i = 0; i < c.length; i++) {
            try {
              ...
              if (message.payload is MqttPublishMessage) {
                ...
                print('MQTT: ⚠️ Mensagem não é do tipo MqttPublishMessage: ${message.payload.runtimeType}');
  ...
  void _processarMensagem(String topic, String payload) {
    try {
      ...
      if (topic.contains(_topicoLocalizacaoOnibus)) {
        ...
        print('MQTT: Tópico contém "_topicoLocalizacaoOnibus": ${topic.contains(_topicoLocalizacaoOnibus)}');
  ...
  void _processarSelecaoBotao(String topic, String payload) {
    try {
      ...
      print('MQTT: Seleção de botão - Parada: $paradaId, Linha: $linha, Tipo: $tipo');
      
      // Armazena a seleção
      if (paradaId.isNotEmpty && linha.isNotEmpty && tipo.isNotEmpty) {
        _selecoesParadas[paradaId] = {
          'linha': linha,
          'tipo': tipo,
        };
    ...
    try {
      ...
      if (botaoStr != null) {
        ...
        if (botao == 'visual' || botao == 'auditivo') {
          ...
          final alerta = jsonData['alerta']?.toString() ?? 'Botão ${botao == 'visual' ? 'Visual' : 'Auditivo'} pressionado';
          
          print('MQTT: Alerta de botão - Linha: $linhaBotao, Tipo: $botao, Alerta: $alerta');
  ...
  void _processarLocalizacaoOnibus(String topic, String payload) {
    try {
      ...
      if (lat != null && lon != null) {
        print('MQTT: ✅ Localização do ônibus válida - Linha: $linha, Lat: $lat, Lon: $lon');
  ...
  void _verificarProximidadeParadas(String linha, double lat, double lon) {
    print('MQTT: Verificando proximidade - Linha: $linha, Lat: $lat, Lon: $lon');
    ...
    _selecoesParadas.forEach((paradaId, selecao) {
      ...
      print('MQTT: Verificando parada $paradaId - Linha selecionada: $linhaSelecionada, Tipo: $tipoDeficiencia');
      
      // Se a linha selecionada corresponde à linha do ônibus
      if (linhaSelecionada == linha) {
        print('MQTT: ✅ Linha $linha corresponde à seleção da parada $paradaId!');
        ...
        if (coordsParada != null) {
          ...
          final distancia = _calcularDistancia(
            lat, lon,
            coordsParada['lat']!, coordsParada['lon']!,
          );
          
          print('MQTT: Distância do ônibus $linha até parada $paradaId: ${distancia.toStringAsFixed(2)}m (limite: ${distanciaProximidade}m)');
          
          // Se está próximo (dentro da distância configurada)
          if (distancia <= distanciaProximidade) {
            print('MQTT: 🚌 Ônibus $linha está próximo! Publicando chegada para parada $paradaId');
            ...
            print('MQTT: ✅ Seleção da parada $paradaId removida após publicar chegada');
          } else {
            print('MQTT: ⏳ Ônibus $linha ainda está longe da parada $paradaId (${distancia.toStringAsFixed(2)}m > ${distanciaProximidade}m)');
          }
        } else {
          // Se não tem coordenadas configuradas, publica imediatamente (modo teste)
          print('MQTT: ⚠️ Parada $paradaId sem coordenadas configuradas');
          print('MQTT: 🚌 Publicando chegada imediatamente (modo teste) para parada $paradaId');
        ...
        print('MQTT: ⏭️ Linha $linha não corresponde à seleção da parada $paradaId (esperava: $linhaSelecionada)');
      }
    });
  ...
  Future<void> _processarAlertaBotao(String linha, String tipoDeficiencia, String alerta) async {
    try {
      print('MQTT: Alerta de botão - Linha: $linha, Tipo: $tipoDeficiencia, Alerta: $alerta');
  ...
  void _processarOnibusChegando(String topic, String payload) {
    try {
      ...
      // Chama callback se configurado
      if (_onOnibusChegando != null && linha.isNotEmpty && paradaId.isNotEmpty) {
  ...
  /// [paradaId] - ID da parada
  Future<void> _verificarENotificarSelecaoBotao(String linha, String tipo, String paradaId) async {
    try {
      if (linha.isEmpty || tipo.isEmpty) return;
      
      print('MQTT: Verificando seleção de botão - Linha: $linha, Tipo: $tipo, Parada: $paradaId');
      ...
      if (linhaSelecionadaApp == null || linhaSelecionadaApp.isEmpty) {
        print('MQTT: ⚠️ Nenhuma linha selecionada no app. Botão pressionado será ignorado.');
        return;
      }
      ...
      if (linhaSelecionadaApp.toUpperCase() != linha.toUpperCase()) {
        print('MQTT: ⚠️ Linha do botão ($linha) não corresponde à linha selecionada no app ($linhaSelecionadaApp). Notificação não será enviada.');
        return;
      }
      
      print('MQTT: ✅ Linha do botão corresponde à linha selecionada! Notificando usuário...');
  ...
  Future<void> _salvarLocalizacaoNoFirebase(String linha, double lat, double lon) async {
    try {
  ...
  Future<void> _salvarChegadaNoFirebase(String linha, String paradaId) async {
    try {
      ...
      print('MQTT: ✅ Chegada salva no Firebase - Parada: $paradaId, Linha: $linha');
    } catch (e, stackTrace) {
  ...
  Future<void> _salvarSelecaoBotaoNoFirebase(String paradaId, String linha, String tipo) async {
    try {
      print('MQTT: Tentando salvar seleção no Firebase - Parada: $paradaId, Linha: $linha, Tipo: $tipo');
      final timestamp = DateTime.now().millisecondsSinceEpoch;
  ...
  void configurarSalvarNoFirebase(bool habilitar) {
    ...
    print('MQTT: Salvamento no Firebase ${habilitar ? "habilitado" : "desabilitado"}');
  }
